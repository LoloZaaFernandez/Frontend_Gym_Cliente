...
def show_asistencia_view(page: ft.Page, auth_service, on_back):
    ...
    def on_dni_change(e):
        """Validar DNI mientras se escribe y auto-buscar"""
    ...
    def buscar_cliente():
        """Buscar cliente por DNI con validaciones"""
        ...
        if not dni.isdigit():
            mostrar_mensaje("⚠ El DNI debe contener solo números", error=True)
            return
        ...
        try:
            ...
            if not cliente:
                mostrar_mensaje(f"❌ Cliente con DNI {dni} no encontrado en el sistema", error=True)
                cliente_info_container.visible = False
            elif cliente.get('estado') != 'Activo':
            ...
            if "Connection" in error_msg:
                ...
                mostrar_mensaje(f"❌ Error al buscar cliente: {error_msg}", error=True)
            cliente_info_container.visible = False
