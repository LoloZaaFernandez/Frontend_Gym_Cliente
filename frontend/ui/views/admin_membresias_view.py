"""
Vista de Gestión de Membresías para Administrador - Versión Mejorada
Con tarjetas visuales similares a la vista del cliente
"""
...
def show_admin_membresias_view(page: ft.Page, auth_service, on_back):
    """Vista mejorada de gestión de membresías con diseño visual"""
    page.clean()
    page.title = "BLESSED GYM - Gestión de Membresías"

    api = APIService()

    # Información visual por tipo de membresía
    membresia_info = {
    ...
    def cargar_membresias():
        ...
        membresias_container.controls.clear()  # ✅ IMPORTANTE: Limpiar antes
        ...
        page.update()  # ✅ Actualizar al final

    def crear_membresia_card(membresia):
        """Crear tarjeta visual de membresía (similar a vista cliente)"""
        tipo = membresia['tipo_membresia']
        info = membresia_info.get(tipo, {"color": PRIMARY_COLOR, "duracion": "N/A", "icono": ft.Icons.CARD_MEMBERSHIP})
        ...
        card_content = [
            ...
            ft.Container(
                content=ft.Row([
                    ft.Icon(
                        ft.Icons.CHECK_CIRCLE if membresia['estado'] == "Activa" else ft.Icons.CANCEL,
                        size=16,
                        color=estado_color
                    ),
                    ft.Text(f"Estado: {estado_text}", size=13, color=estado_color, weight=ft.FontWeight.BOLD),
                ...
                content=ft.Column([
                    ft.Row([
                        ft.ElevatedButton(
                            ...
                            on_click=lambda e, m=membresia: editar_membresia(m),
                            expand=True,
                            height=40
                        ),
                        ft.ElevatedButton(
                            ...
                            on_click=lambda e, m=membresia: gestionar_precio(m),
                            expand=True,
                            height=40
                        ),
                    ], spacing=10),
                    ft.ElevatedButton(
                        "Desactivar" if membresia['estado'] == "Activa" else "Activar",
                        icon=ft.Icons.TOGGLE_OFF if membresia['estado'] == "Activa" else ft.Icons.TOGGLE_ON,
                        style=ft.ButtonStyle(
                            color=ft.Colors.WHITE,
                            bgcolor="#ef5350" if membresia['estado'] == "Activa" else "#4CAF50",
                        ),
                        on_click=lambda e, m=membresia: cambiar_estado(m),
                        width=260,
                        height=40
                    ),
                ], spacing=10),
        ...
        return ft.Container(
            ...
            border_radius=12,
            padding=20,
            border=ft.border.all(2 if membresia['estado'] == "Activa" else 1, info['color'] if membresia['estado'] == "Activa" else "#333333"),
            shadow=ft.BoxShadow(
                spread_radius=1 if membresia['estado'] == "Activa" else 0,
                blur_radius=15 if membresia['estado'] == "Activa" else 10,
                color=f"{info['color']}44" if membresia['estado'] == "Activa" else ft.Colors.BLACK26,
