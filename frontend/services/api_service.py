...
class APIService:
    ...
    def get_membresias(self) -> List[Dict[str, Any]]:
        """Obtener lista de membresías disponibles"""
        url = f"{self.base_url}/api/membresias"
        try:
            response = self.session.get(url, timeout=self.timeout)
            return self._handle_response(response)
    ...
    def asignar_membresia(self, cliente_id: int, membresia_id: int) -> Dict[str, Any]:
        """Asignar membresía a un cliente"""
        url = f"{self.base_url}/api/membresias/asignar"
        data = {
            "cliente_id": cliente_id,
        ...
        try:
            ...
            raise ValueError(f"Error al asignar membresía: {str(e)}")
