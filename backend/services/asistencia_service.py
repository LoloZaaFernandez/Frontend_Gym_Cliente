...
class AsistenciaService:
    ...
    def registrar_asistencia(self, asistencia_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cliente_info = self.repository.get_cliente_info_by_dni(
            asistencia_data['dni'])
        ...
        nombre_completo = f"{cliente_info['nombre']} {
            cliente_info['apellidos']}"
    ...
    def listar_asistencias(self, fecha_inicio: Optional[str] = None, fecha_fin: Optional[str] = None,
                           cliente_id: Optional[int] = None, skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
        return self.repository.find_all(fecha_inicio, fecha_fin, cliente_id, skip, limit)
    ...
    def obtener_estadisticas_dia(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "asistencias": asistencias_hoy[:10]  # Últimas 10 para preview
        }

    def obtener_estadisticas_mes(self) -> Dict[str, Any]:
        """Obtener estadísticas de asistencias del mes actual"""
        fecha_actual = datetime.now()
        fecha_inicio = fecha_actual.replace(day=1).date().isoformat()
        fecha_fin = fecha_actual.date().isoformat()

        asistencias_mes = self.repository.find_all(fecha_inicio, fecha_fin, None, 0, 10000)

        # Contar asistencias únicas por cliente
        clientes_unicos = set()
        for asistencia in asistencias_mes:
            clientes_unicos.add(asistencia.get('id_cliente'))

        return {
            "mes": fecha_actual.strftime("%Y-%m"),
            "total_asistencias": len(asistencias_mes),
            "clientes_activos": len(clientes_unicos),
    ...
    def obtener_estadisticas_cliente(self, cliente_id: int) -> Dict[str, Any]:
        ...
        cliente_info = self.repository.get_cliente_info(cliente_id)
        if not cliente_info:
        ...
        fecha_inicio_mes = datetime.now().replace(day=1).date().isoformat()
        asistencias_mes = self.repository.find_all(fecha_inicio_mes, None, cliente_id, 0, 1000)

        # Última asistencia
        ultima = asistencias[0] if asistencias else None

        return {
            "cliente_id": cliente_id,
            "nombre_cliente": cliente_info.get('nombre', '') + ' ' + cliente_info.get('apellidos', ''),
            "total_asistencias": len(asistencias),
            "ultima_asistencia": ultima.get('fecha_asistencia') if ultima else None,
            "asistencias_mes_actual": len(asistencias_mes),
            "asistencias_recientes": asistencias[:10]  # Últimas 10
        }
