...
def obtener_cliente_por_dni(dni: str):
    ...
    try:
        return service.obtener_cliente_por_dni(dni)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))


@router.put("/dni/{dni}", response_model=ClienteResponse)
def actualizar_cliente_por_dni(dni: str, cliente: ClienteUpdate):
    """Actualizar un cliente por DNI"""
    try:
        return service.actualizar_cliente_por_dni(dni, cliente.dict(exclude_unset=True))
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))


@router.delete("/dni/{dni}", status_code=status.HTTP_204_NO_CONTENT)
def eliminar_cliente_por_dni(dni: str):
    """Eliminar (desactivar) un cliente por DNI"""
    try:
        service.eliminar_cliente_por_dni(dni)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
