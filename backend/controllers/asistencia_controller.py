...
@router.get("/dni/{dni}", response_model=List[AsistenciaResponse])
def listar_asistencias_por_dni(dni: str, skip: int = 0, limit: int = 50):
    """Listar asistencias de un cliente por DNI"""
    try:
        return service.listar_asistencias_por_dni(dni, skip, limit)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))


@router.get("/dni/{dni}/estadisticas")
def obtener_estadisticas_por_dni(dni: str):
    """Obtener estadísticas de asistencias por DNI"""
    try:
        return service.obtener_estadisticas_cliente_por_dni(dni)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
